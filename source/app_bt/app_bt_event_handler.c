...
app_bt_management_callback(wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data)
{
    ...
    wiced_bt_device_address_t local_bda = {0x22, 0xA0, 0x50, 0x012, 0x34, 0xCC};
    ...
    switch (event)
    {
        ...
        case BTM_BLE_ADVERT_STATE_CHANGED_EVT:
            ...
            if (BTM_BLE_ADVERT_OFF == *p_mode && app_bt_run_mode == APP_BT_RUN_MODE_EXCHANGE_EAD_KEYS)
            {
                app_bt_adv_stop_handler();
            }
            else
            {
                ble_ead_advertiser_state_changed(event, p_event_data);
            }
            break;
        ...
        case BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT:
            ...
            //Generate the EAD Keys
            app_bt_update_ead_keys(app_bt_generate_ead_keys());
            break;
...
void app_bt_application_init(void)
{
    ...
    printf("\nBluetooth LE EAD Advertiser application init\n");
    ...
    if(app_bt_restore_ead_keys() != CY_RSLT_SUCCESS)
    {
        ...
        /* If local not bond with peer and no BLE EAD keys start connectable advertisements */
        /* Allow peer to pair */
    ...
    else
    {
        ...
        /* Start BLE EAD non-connectable advertisments */
