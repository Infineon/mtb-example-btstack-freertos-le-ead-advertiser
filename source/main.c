...
*              LE EAD advertiser Example for ModusToolbox.
*
...
int main()
{
    ...
    printf("*************************"
           "Bluetooth LE EAD Advertiser Application Start "
           "**************************\n");
    ...
    /* Problem - there can be race condition where the bluetooth stack enable can invoke app_kv_store_init
    * in a state where a context switch may occur and corrupt the flash initialization.
    * Workaround and best practice guidance is for applications to call app_kv_store_init
    * before wiced_bt_stack_init
    */
    app_kv_store_init();
