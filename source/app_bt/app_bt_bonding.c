...
wiced_bt_ble_key_material_t ble_ead_key_material;
...
#ifdef LINK_KEY_DUMP
    if(link_key->key_data.le_keys_available_mask & BTM_LE_KEY_LENC)
    {
        printf("lltk:");
        print_array(link_key->key_data.le_keys.lltk, sizeof(BT_OCTET16));
    }
#endif
...
/**
* Function Name:
* app_bt_restore_ead_keys
*
* Function Description:
* @brief This function restore the BLE EAD key material from kv store
*
* @param None
*
* @return  cy_rslt_t: CY_RSLT_SUCCESS if the restore was successful,
*              an error code otherwise.
*/
cy_rslt_t app_bt_restore_ead_keys(void)
{
    cy_rslt_t rslt = CY_RSLT_TYPE_ERROR;
    uint32_t data_size = sizeof(wiced_bt_ble_key_material_t);
    wiced_bt_ble_key_material_t read_ead_key;

#ifdef LINK_KEY_DUMP
    wiced_bt_link_key_t* p_key = NULL;
    wiced_bt_iv_t*p_iv = NULL;
#endif

    rslt = mtb_kvstore_read(&kvstore_obj, "ead_key_material", (uint8_t *)&read_ead_key, &data_size);

    if(rslt == CY_RSLT_SUCCESS)
    {
        memcpy(&ble_ead_key_material, &read_ead_key, sizeof(wiced_bt_ble_key_material_t));
    }

#ifdef LINK_KEY_DUMP
    p_key = &ble_ead_key_material.session_key;
    p_iv = &ble_ead_key_material.iv;

    printf("\nRestore BLE EAD Keys: \n");
    printf("session_key:");
    print_array(p_key, sizeof(wiced_bt_link_key_t));
    printf("iv:");
    print_array(p_iv, sizeof(wiced_bt_iv_t));
#endif
...
/**
* Function Name:
* app_bt_update_ead_keys
*
* Function Description:
* @brief This function update the BLE EAD key material into kv store
*
* @param None
*
* @return  cy_rslt_t: CY_RSLT_SUCCESS if the update was successful,
*              an error code otherwise.
*/
cy_rslt_t app_bt_update_ead_keys(wiced_bt_ble_key_material_t* p_key_material)
{
    cy_rslt_t rslt = CY_RSLT_TYPE_ERROR;
    ...
    if(p_key_material != NULL)
    {
        printf("\nUpdating BLE EAD Key Material\n");
        rslt = mtb_kvstore_write(&kvstore_obj, "ead_key_material", (uint8_t *)p_key_material, sizeof(wiced_bt_ble_key_material_t));

        if(rslt == CY_RSLT_SUCCESS)
        {
            //Update the EAD keys value into attr
            app_bt_set_value(HDLC_HELLO_SENSOR_EAD_KEY_MATERIAL_VALUE, (uint8_t *)p_key_material, sizeof(wiced_bt_ble_key_material_t));
        }
    }

#ifdef LINK_KEY_DUMP
    p_key = &p_key_material->session_key;
    p_iv = &p_key_material->iv;
...
/**
* Function Name:
* app_bt_generate_ead_keys
*
* Function Description:
* @brief This function generate one random BLE EAD key material into ble_ead_key_material
*
* @param None
*
* @return wiced_bt_ble_key_material_t* : generated the random BLE EAD Keys
*/
wiced_bt_ble_key_material_t* app_bt_generate_ead_keys(void)
{
    cy_en_cryptolite_status_t cry_rslt = CY_CRYPTOLITE_SUCCESS;
    wiced_bt_ble_key_material_t* p_key_material = NULL;
    uint8_t randomsize = 0;
    ...
    {
        cry_rslt = Cy_Cryptolite_Trng_Enable(CRYPTOLITE);
        if(cry_rslt == CY_CRYPTOLITE_SUCCESS)
        {
            randomsize = sizeof(wiced_bt_link_key_t);
            temp_ptr = &ble_ead_key_material.session_key[0];
            while ((randomsize > 0) && (CY_CRYPTOLITE_SUCCESS == cry_rslt))
            {
                cry_rslt = Cy_Cryptolite_Trng_ReadData(CRYPTOLITE, (uint32_t *)temp_ptr);
                temp_ptr += 4u;
                randomsize -= 4;

                if (CY_CRYPTOLITE_SUCCESS != cry_rslt)
                {
                    break;
                }
            }

            randomsize = sizeof(wiced_bt_iv_t);
            temp_ptr = &ble_ead_key_material.iv[0];
            while ((randomsize > 0) && (CY_CRYPTOLITE_SUCCESS == cry_rslt))
            {
                cry_rslt = Cy_Cryptolite_Trng_ReadData(CRYPTOLITE, (uint32_t *)temp_ptr);
                temp_ptr += 4u;
            ...
            p_key_material = &ble_ead_key_material;

#ifdef LINK_KEY_DUMP
            p_key = &ble_ead_key_material.session_key;
            p_iv = &ble_ead_key_material.iv;

            printf("\nGenerate Random BLE EAD Keys: \n");
            printf("session_key:");
            print_array(p_key, sizeof(wiced_bt_link_key_t));
            printf("iv:");
            print_array(p_iv, sizeof(wiced_bt_iv_t));
        ...
        {
            (void)Cy_Cryptolite_Trng_DeInit(CRYPTOLITE);
            printf("Cy_Cryptolite_Trng_Enable fail\n");
        }
    }
    else
    {
        printf("Generate Random BLE EAD Keys failure\n");
    }

    return p_key_material;
}
