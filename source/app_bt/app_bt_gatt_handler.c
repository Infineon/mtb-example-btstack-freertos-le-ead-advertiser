                             ...
                             uint16_t *p_error_handle)
{
   ...
   *p_error_handle = p_read_req->handle;
    ...
    /* If peer client read the EAD, trigger a timer to restart */
    if(puAttribute->handle == HDLC_HELLO_SENSOR_EAD_KEY_MATERIAL_VALUE)
    {
        ...
        if (app_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            ...
            if (validLen)
            {
                ...
                *  peer can enable or disable notification or indication */
                ...
                case HDLD_HELLO_SENSOR_EAD_KEY_MATERIAL_CLIENT_CHAR_CONFIG:
                    gatt_status = WICED_BT_GATT_SUCCESS;
                    break;
